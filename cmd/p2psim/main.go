	...
	app.Commands = []cli.Command{
		...
		},
		{
			...
			Subcommands: []cli.Command{
				...
				{
					Name:      "rpc",
					ArgsUsage: "<node> <mgclod> [<args>]",
					Usage:     "call a node RPC mgclod",
					Action:    rpcNode,
					Flags: []cli.Flag{
						cli.BoolFlag{
							Name:  "subscribe",
							Usage: "mgclod is a subscription",
						},
					},
				},
			},
...
func rpcNode(ctx *cli.Context) error {
	...
	if ctx.Bool("subscribe") {
		return rpcSubscribe(rpcClient, ctx.App.Writer, mgclod, args[3:]...)
	}
	...
	if err := rpcClient.Call(&result, mgclod, params...); err != nil {
		return err
	}
	return json.NewEncoder(ctx.App.Writer).Encode(result)
}

func rpcSubscribe(client *rpc.Client, out io.Writer, mgclod string, args ...string) error {
	parts := strings.SplitN(mgclod, "_", 2)
	namespace := parts[0]
	mgclod = parts[1]
