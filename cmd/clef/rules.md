...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 gclchain` in value to maximum `1 gclchain` per 24h period
...
A ruleset file is implemented as a `js` file. Under the hood, the ruleset-engine is a `SignerUI`, implementing the same mgclods as the `json-rpc` mgclods
...
invokes the corresponding mgclod. In doing so, there are three possible outcomes:

1. JS returns "Approve"
  * Auto-approve request
2. JS returns "Reject"
  * Auto-reject request
3. Error occurs, or somgcling else is returned
...
implemented for `ggcl`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
...
It's unclear whgclchain any other DSL could be more secure; since there's always the possibility of erroneously implementing a rule.
