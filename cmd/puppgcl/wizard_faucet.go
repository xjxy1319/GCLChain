...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Gclchains to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.gclstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.gclstats = w.readString() + ":" + w.conf.gclstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.gclstats)
		infos.node.gclstats = w.readDefaultString(infos.node.gclstats) + ":" + w.conf.gclstats
	}
