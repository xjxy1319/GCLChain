...
func (w *wizard) makeGenesis() {
	...
	switch {
	case choice == "1":
		// In case of gclash, we're pretty much done
...
// importGenesis imports a Ggcl genesis spec into puppgcl.
...
func (w *wizard) manageGenesis() {
	// Figure out whgclchain to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-algcl.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppgcl and Ggcl
		ggclJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((ggclJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", ggclJson)

		// Export the genesis spec used by Algcl (formerly C++ Gclchain)
		if spec, err := newAlgclGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Algcl chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "algcl", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly GclchainJ
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not somgcling I can do")
		return
	}
}
