...
// nodeDockerfile is the Dockerfile required to run an Gclchain node.
var nodeDockerfile = `
FROM gclchaineum/client-go:latest
...
{{end}}
RUN \
  echo 'ggcl --cache 512 init /genesis.json' > ggcl.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.gclchaineum/keystore/ && cp /signer.json /root/.gclchaineum/keystore/' >> ggcl.sh && \{{end}}
	echo $'exec ggcl --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --gclstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Gclchainbase}}--miner.gclchainbase {{.Gclchainbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> ggcl.sh

ENTRYPOINT ["/bin/sh", "ggcl.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Gclchain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.gclchaineum{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.gclash{{end}}
    environment:
      - PORT={{.Port}}/tcp
      - TOTAL_PEERS={{.TotalPeers}}
      - LIGHT_PEERS={{.LightPeers}}
      - STATS_NAME={{.Ethstats}}
      - MINER_NAME={{.Gclchainbase}}
...
// deployNode deploys a new Gclchain node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.gclchainbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Ethashdir":  config.gclashdir,
		"Network":    network,
		"Port":       config.port,
		"TotalPeers": config.peersTotal,
		"Light":      config.peersLight > 0,
		"LightPeers": config.peersLight,
		"Ethstats":   config.gclstats[:strings.Index(config.gclstats, ":")],
		"Gclchainbase":  config.gclchainbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Ethstats username":        info.gclstats,
	}
	if info.gasTarget > 0 {
		...
		if info.gclchainbase != "" {
			// Ethash proof-of-work miner
			report["Ethash directory"] = info.gclashdir
			report["Miner account"] = info.gclchainbase
		}
		if info.keyJSON != "" {
...
// whgclchain it's running, and if yes, whgclchain it's responsive.
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 ggcl --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.gclchaineum"],
		gclashdir:  infos.volumes["/root/.gclash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		gclstats:   infos.envvars["STATS_NAME"],
		gclchainbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasLimit:   gasLimit,
