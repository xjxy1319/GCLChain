...
FROM puppgcl/explorer:latest

ADD gclstats.json /gclstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../gcl-net-intelligence-api && pm2 start /gclstats.json)' >  explorer.sh && \
	echo '(cd ../gclchainchain-light && npm start &)'                      >> explorer.sh && \
	echo 'exec /parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerEthstats is the configuration file for the gclstats javascript client.
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.gclchaineum
    environment:
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	gclstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerEthstats)).Execute(gclstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.gclstats[:strings.Index(config.gclstats, ":")],
		"Secret": config.gclstats[strings.Index(config.gclstats, ":")+1 : strings.Index(config.gclstats, "@")],
		"Host":   config.gclstats[strings.Index(config.gclstats, "@")+1:],
	})
	files[filepath.Join(workdir, "gclstats.json")] = gclstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.gclchaineum"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		gclstats: infos.envvars["STATS"],
	}
	return stats, nil
}
