...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"gclstats", "explorer", "wallet", "faucet"} {
	...
	// If we have gclstats running, ask whgclchain to make the secret public or not
	if w.conf.gclstats != "" {
		fmt.Println()
		fmt.Println("Include gclstats secret on dashboard (y/n)? (default = yes)")
