...
import (
	...
	math2 "github.com/gclchaineum/go-gclchaineum/common/math"
...
// algclGenesisSpec represents the genesis specification format used by the
// C++ Gclchain implementation.
type algclGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*algclGenesisSpecAccount `json:"accounts"`
}

// algclGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type algclGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256   `json:"balance"`
	Nonce       uint64                   `json:"nonce,omitempty"`
	Precompiled *algclGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// algclGenesisSpecBuiltin is the precompiled contract definition.
type algclGenesisSpecBuiltin struct {
	Name          string                         `json:"name,omitempty"`
	StartingBlock hexutil.Uint64                 `json:"startingBlock,omitempty"`
	Linear        *algclGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type algclGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newAlgclGenesisSpec converts a go-gclchaineum genesis block into a Algcl-specific
// chain specification format.
func newAlgclGenesisSpec(network string, genesis *core.Genesis) (*algclGenesisSpec, error) {
	// Only gclash is currently supported between go-gclchaineum and algcl
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Algcl format
	spec := &algclGenesisSpec{
		SealEngine: "Ethash",
	}
	// Some defaults
	...
	spec.Params.BlockReward = (*hexutil.Big)(gclash.FrontierBlockReward)
	...
	spec.setPrecompile(1, &algclGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &algclGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &algclGenesisSpecBuiltin{Name: "sha256",
		Linear: &algclGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &algclGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &algclGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &algclGenesisSpecBuiltin{Name: "identity",
		Linear: &algclGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &algclGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64())})
		spec.setPrecompile(6, &algclGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
			Linear:        &algclGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &algclGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
			Linear:        &algclGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &algclGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64())})
	}
	return spec, nil
}

func (spec *algclGenesisSpec) setPrecompile(address byte, data *algclGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*algclGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &algclGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *algclGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*algclGenesisSpecAccount)
	}
...
func (spec *algclGenesisSpec) setByzantium(num *big.Int) {
	spec.Params.ByzantiumForkBlock = hexutil.Uint64(num.Uint64())
}

func (spec *algclGenesisSpec) setConstantinople(num *big.Int) {
...
// newParityChainSpec converts a go-gclchaineum genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only gclash is currently supported between go-gclchaineum and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	// ggcl has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.Gclchain.Nonce = (hexutil.Bytes)(make([]byte, 8))
	binary.LittleEndian.PutUint64(spec.Genesis.Seal.Gclchain.Nonce[:], genesis.Nonce)

	spec.Genesis.Seal.Gclchain.MixHash = (hexutil.Bytes)(genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
func (spec *parityChainSpec) setByzantium(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(gclash.ByzantiumBlockReward)
...
func (spec *parityChainSpec) setConstantinople(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(gclash.ConstantinopleBlockReward)
...
// pyGclchainGenesisSpec represents the genesis specification format used by the
// Python Gclchain implementation.
type pyGclchainGenesisSpec struct {
	Nonce      hexutil.Bytes     `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyGclchainGenesisSpec converts a go-gclchaineum genesis block into a Parity specific
// chain specification format.
func newPyGclchainGenesisSpec(network string, genesis *core.Genesis) (*pyGclchainGenesisSpec, error) {
	// Only gclash is currently supported between go-gclchaineum and pygclchaineum
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyGclchainGenesisSpec{
