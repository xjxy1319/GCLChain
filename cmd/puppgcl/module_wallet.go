...
FROM puppgcl/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'ggcl --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec ggcl --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --gclstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppgclNetworkID/{{.NetworkID}}/g' dist/js/gclchainwallet-master.js && \
	sed -i 's/PuppgclNetwork/{{.Network}}/g'     dist/js/gclchainwallet-master.js && \
	sed -i 's/PuppgclDenom/{{.Denom}}/g'         dist/js/gclchainwallet-master.js && \
	sed -i 's/PuppgclHost/{{.Host}}/g'           dist/js/gclchainwallet-master.js && \
	sed -i 's/PuppgclRPCPort/{{.RPCPort}}/g'     dist/js/gclchainwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whgclchain
// it's running, and if yes, whgclchain it's responsive.
