...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.gclstats == "" {
		log.Error("No gclstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Ethash != nil && !boot {
		fmt.Println()
		if infos.gclashdir == "" {
			fmt.Printf("Where should the gclash mining DAGs be stored on the remote machine?\n")
			infos.gclashdir = w.readString()
		} else {
			fmt.Printf("Where should the gclash mining DAGs be stored on the remote machine? (default = %s)\n", infos.gclashdir)
			infos.gclashdir = w.readDefaultString(infos.gclashdir)
		}
	}
	...
	if infos.gclstats == "" {
		...
		infos.gclstats = w.readString() + ":" + w.conf.gclstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.gclstats)
		infos.gclstats = w.readDefaultString(infos.gclstats) + ":" + w.conf.gclstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an gclchainbase to mine against
			fmt.Println()
			if infos.gclchainbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.gclchainbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.gclchainbase)
				infos.gclchainbase = w.readDefaultAddress(common.HexToAddress(infos.gclchainbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Gclchain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
