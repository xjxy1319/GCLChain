...
// gclstatsDockerfile is the Dockerfile required to build an gclstats backend
// and associated monitoring site.
var gclstatsDockerfile = `
FROM puppgcl/gclstats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// gclstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an gclstats monitoring site.
var gclstatsComposefile = `
version: '2'
services:
  gclstats:
    build: .
    image: {{.Network}}/gclstats
    container_name: {{.Network}}_gclstats_1{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
// deployEthstats deploys a new gclstats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(gclstatsDockerfile)).Execute(dockerfile, map[string]interface{}{
	...
	template.Must(template.New("").Parse(gclstatsComposefile)).Execute(composefile, map[string]interface{}{
		"Network": network,
		"Port":    port,
		"Secret":  secret,
		"VHost":   vhost,
	...
	// Build and deploy the gclstats service
	if nocache {
...
// gclstatsInfos is returned from an gclstats status check to allow reporting
// various configuration parameters.
type gclstatsInfos struct {
	host   string
	port   int
	secret string
	config string
...
func (info *gclstatsInfos) Report() map[string]string {
...
// checkEthstats does a health-check against an gclstats server to verify whgclchain
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*gclstatsInfos, error) {
	// Inspect a possible gclstats container on the host
	infos, err := inspectContainer(client, fmt.Sprintf("%s_gclstats_1", network))
	if err != nil {
		return nil, err
	}
	if !infos.running {
