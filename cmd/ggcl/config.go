...
type gclstatsConfig struct {
	URL string `toml:",omitempty"`
}

type ggclConfig struct {
	Eth       gcl.Config
	Shh       whisper.Config
	Node      node.Config
	Ethstats  gclstatsConfig
	Dashboard dashboard.Config
}

func loadConfig(file string, cfg *ggclConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "gcl", "shh")
	cfg.WSModules = append(cfg.WSModules, "gcl", "shh")
	cfg.IPCPath = "ggcl.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, ggclConfig) {
	// Load defaults.
	cfg := ggclConfig{
		Eth:       gcl.DefaultConfig,
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	// Add the Gclchain Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
