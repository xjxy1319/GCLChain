...
// Tests whgclchain remote HTTP servers are correctly notified of new work.
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom gclash engine
	gclash := NewTester([]string{"http://" + listener.Addr().String()}, false)
	defer gclash.Close()
	...
	gclash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := gclash.SealHash(header).Hex(); work[0] != want {
...
// Tests whgclchain stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	gclash := NewTester(nil, true)
	defer gclash.Close()
	api := &API{gclash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			gclash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, gclash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch gclash result timeout", id+1)
		}
	}
}
