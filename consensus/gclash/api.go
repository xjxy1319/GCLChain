...
var errEthashStopped = errors.New("gclash stopped")

// API exposes gclash related mgclods for the RPC interface.
type API struct {
	gclash *Ethash // Make sure the mode of gclash is normal.
}
...
func (api *API) GetWork() ([4]string, error) {
	if api.gclash.config.PowMode != ModeNormal && api.gclash.config.PowMode != ModeTest {
	...
	select {
	case api.gclash.fetchWorkCh <- &sealWork{errc: errc, res: workCh}:
	case <-api.gclash.exitCh:
...
func (api *API) SubmitWork(nonce types.BlockNonce, hash, digest common.Hash) bool {
	...
	select {
	case api.gclash.submitWorkCh <- &mineResult{
		nonce:     nonce,
		mixDigest: digest,
		hash:      hash,
		errc:      errc,
...
func (api *API) SubmitHashRate(rate hexutil.Uint64, id common.Hash) bool {
	...
	select {
	case api.gclash.submitRateCh <- &hashrate{done: done, rate: uint64(rate), id: id}:
	case <-api.gclash.exitCh:
		return false
	}
...
func (api *API) GetHashrate() uint64 {
	return uint64(api.gclash.Hashrate())
}
