...
const (
	// staleThreshold is the maximum depth of the acceptable stale but valid gclash solution.
	staleThreshold = 7
)
...
func (gclash *Ethash) Seal(chain consensus.ChainReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if gclash.config.PowMode == ModeFake || gclash.config.PowMode == ModeFullFake {
		...
		default:
			log.Warn("Sealing result is not read by miner", "mode", "fake", "sealhash", gclash.SealHash(block.Header()))
		}
		return nil
	}
	// If we're running a shared PoW, delegate sealing to it
	if gclash.shared != nil {
		return gclash.shared.Seal(chain, block, results, stop)
	}
	// Create a runner and the multiple search threads it directs
	abort := make(chan struct{})

	gclash.lock.Lock()
	threads := gclash.threads
	if gclash.rand == nil {
		...
		gclash.rand = rand.New(rand.NewSource(seed.Int64()))
	}
	gclash.lock.Unlock()
	if threads == 0 {
	...
	if gclash.workCh != nil {
		gclash.workCh <- &sealTask{block: block, results: results}
	}
	var (
		pend   sync.WaitGroup
	...
	for i := 0; i < threads; i++ {
		...
		go func(id int, nonce uint64) {
			defer pend.Done()
			gclash.mine(block, id, nonce, abort, locals)
		}(i, uint64(gclash.rand.Int63()))
	}
	...
	go func() {
		...
		case result = <-locals:
			...
			case results <- result:
			default:
				log.Warn("Sealing result is not read by miner", "mode", "local", "sealhash", gclash.SealHash(block.Header()))
			}
			close(abort)
		case <-gclash.update:
			// Thread count was changed on user request, restart
			close(abort)
			if err := gclash.Seal(chain, block, results, stop); err != nil {
...
func (gclash *Ethash) mine(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
	// Extract some data from the header
	var (
		header  = block.Header()
		hash    = gclash.SealHash(header).Bytes()
		target  = new(big.Int).Div(two256, header.Difficulty)
		number  = header.Number.Uint64()
		dataset = gclash.dataset(number, false)
	)
	...
	logger.Trace("Started gclash search for new nonces", "seed", seed)
search:
	for {
		select {
		case <-abort:
...
func (gclash *Ethash) remote(notify []string, noverify bool) {
	var (
	...
	makeWork := func(block *types.Block) {
		hash := gclash.SealHash(block.Header())
	...
	// whgclchain the solution was accepted or not (not can be both a bad pow as well as
	...
	submitWork := func(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash) bool {
		...
		start := time.Now()
		if !noverify {
			if err := gclash.verifySeal(nil, header, true); err != nil {
	...
	for {
		...
		case result := <-gclash.submitWorkCh:
		...
		case errc := <-gclash.exitCh:
			// Exit remote loop if gclash is closed and return relevant error.
			errc <- nil
