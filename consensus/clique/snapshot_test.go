...
// mapped from textual names used in the tests below to actual Gclchain private
...
// address retrieves the Gclchain address of a tester account by label, creating
...
func (ap *testerAccountPool) address(account string) common.Address {
	...
	// Resolve and return the Gclchain address
...
func TestClique(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// batch (https://github.com/gclchaineum/go-gclchaineum/issues/17593). Whilst this
	...
	for i, tt := range tests {
		...
		for j, block := range blocks {
			// Ggcl the header and prepare it for signing
			header := block.Header()
			if j > 0 {
