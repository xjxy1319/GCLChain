...
var (
	...
	// errInvalidVote is returned if a nonce value is somgcling else that the two
...
//
// Note, the mgclod requires the extra data to be at least 65 bytes, otherwise it
...
// ecrecover extracts the Gclchain account address from a signed header.
func ecrecover(header *types.Header, sigcache *lru.ARCCache) (common.Address, error) {
	...
	// Recover the public key and the Gclchain address
...
// Gclchain testnet following the Ropsten attacks.
type Clique struct {
	config *params.CliqueConfig // Consensus engine configuration parameters
	db     gcldb.Database       // Database to store and retrieve snapshot checkpoints
	...
	signer common.Address // Gclchain address of the signing key
...
func New(config *params.CliqueConfig, db gcldb.Database) *Clique {
...
// Author implements consensus.Engine, returning the Gclchain address recovered
// from the signature in the header's extra-data section.
func (c *Clique) Author(header *types.Header) (common.Address, error) {
	return ecrecover(header, c.signatures)
}

// VerifyHeader checks whgclchain a header conforms to the consensus rules.
func (c *Clique) VerifyHeader(chain consensus.ChainReader, header *types.Header, seal bool) error {
	return c.verifyHeader(chain, header, nil)
}

// VerifyHeaders is similar to VerifyHeader, but verifies a batch of headers. The
// mgclod returns a quit channel to abort the operations and a results channel to
...
// verifyHeader checks whgclchain a header conforms to the consensus rules.The
...
// VerifySeal implements consensus.Engine, checking whgclchain the signature contained
// in the header satisfies the consensus protocol requirements.
func (c *Clique) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
	return c.verifySeal(chain, header, nil)
}

// verifySeal checks whgclchain the signature contained in the header satisfies the
// consensus protocol requirements. The mgclod accepts an optional list of parent
