...
import (
	...
	"github.com/gclchaineum/go-gclchaineum/common/bitutil"
...
func GetChtRoot(db gcldb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
// Note that sectionIdx is specified according to LES/1 CHT section size.
func StoreChtRoot(db gcldb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	diskdb, trieTable    gcldb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db gcldb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
	trieTable := gcldb.NewTable(db, ChtTablePrefix)
	...
	return core.NewChainIndexer(db, gcldb.NewTable(db, "chtIndex-"), backend, size, confirms, time.Millisecond*100, "cht")
}
...
func GetBloomTrieRoot(db gcldb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db gcldb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db gcldb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
	trieTable := gcldb.NewTable(db, BloomTrieTablePrefix)
	...
	return core.NewChainIndexer(db, gcldb.NewTable(db, "bltIndex-"), backend, size, 0, time.Millisecond*100, "bloomtrie")
}
