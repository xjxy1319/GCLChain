...
creating a collection of devp2p nodes, connecting them toggclchain to form a
...
if a node is created without an explicit service), exposes mgclods for
...
connecting two nodes toggclchain).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of somgcling happening in the network as a result of a
...
With a running simulation network, the `Simulation.Run` mgclod can be called
with a `Step` which has the following fields:

* `Action` - a function which performs some action in the network

* `Expect` - an expectation function which returns whgclchain or not a
...
As a concrete example, consider a simulated network of Gclchain nodes. An
...
On return, the `Simulation.Run` mgclod returns a `StepResult` which can be used
...
p2psim node rpc <node> <mgclod> [<args>] [--subscribe]
```
