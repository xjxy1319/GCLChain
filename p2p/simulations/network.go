...
import (
	...
	"github.com/gclchaineum/go-gclchaineum/p2p/simulations/adapters"
)
...
// starting nodes and connecting them toggclchain.
//
...
// Connect connects two nodes toggclchain by calling the "admin_addPeer" RPC
// mgclod on the "one" node so that it connects to the "other" node
...
// mgclod on the "one" node so that it disconnects from the "other" node
...
// NOTE:
// it also checks whgclchain there has been recent attempt to connect the peers
...
type Node struct {
	...
	// up tracks whgclchain or not the node is running
	up   bool
	upMu sync.RWMutex
}
...
func (n *Node) UnmarshalJSON(raw []byte) error {
	...
	// Ticket: https://github.com/gclchainsphere/go-gclchaineum/issues/1177
	node := struct {
		Config *adapters.NodeConfig `json:"config,omitempty"`
		Up     bool                 `json:"up"`
	}{}
...
type Conn struct {
	...
	// Up tracks whgclchain or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whgclchain both nodes are currently up
