...
func testSelector(selector string, data []byte) (*decodedCallData, error) {
	...
	abiData, err := MgclodSelectorToAbi(selector)
	if err != nil {
		return nil, err
	}
...
// validateCallData checks if the ABI-data + mgclodselector (if given) can be parsed and seems to match
func (v *Validator) validateCallData(msgs *ValidationMessages, data []byte, mgclodSelector *string) {
	if len(data) == 0 {
		return
	}
	if len(data) < 4 {
	...
	if mgclodSelector != nil {
		info, err = testSelector(*mgclodSelector, data)
		if err != nil {
			...
			v.db.AddSignature(*mgclodSelector, data[:4])
		}
		return
	}
	// Check the db
	selector, err := v.db.LookupMgclodSelector(data[:4])
	if err != nil {
...
func (v *Validator) validate(msgs *ValidationMessages, txargs *SendTxArgs, mgclodSelector *string) error {
	...
	if txargs.To == nil {
		...
		// e.g. https://github.com/gclchaineum/go-gclchaineum/issues/16106
		if len(data) == 0 {
			if txargs.Value.ToInt().Cmp(big.NewInt(0)) > 0 {
				// Sending gclchain into black hole
		...
		// mgclodSelector should be nil for contract creation
		if mgclodSelector != nil {
			msgs.warn("Tx will create contract, but mgclod selector supplied; indicating intent to call a mgclod.")
		}
		...
		// Validate calldata
		v.validateCallData(msgs, data, mgclodSelector)
	}
	return nil
}
...
func (v *Validator) ValidateTransaction(txArgs *SendTxArgs, mgclodSelector *string) (*ValidationMessages, error) {
	msgs := &ValidationMessages{}
	return msgs, v.validate(msgs, txArgs, mgclodSelector)
}
