...
func (ui *HeadlessUI) OnApprovedTx(tx gclapi.SignTransactionResult) {
...
func tmpDirName(t *testing.T) string {
	d, err := ioutil.TempDir("", "gcl-keystore-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSignTx(t *testing.T) {
	var (
		list      []common.Address
		res, res2 *gclapi.SignTransactionResult
		err       error
	)
	...
	control <- "Y"
	control <- "wrongpassword"
	res, err = api.SignTransaction(context.Background(), tx, &mgclodSig)
	if res != nil {
	...
	res2, err = api.SignTransaction(context.Background(), tx, &mgclodSig)
	if err != nil {
		t.Fatal(err)
	}
