...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, mgclodSelector *string) (*gclapi.SignTransactionResult, error)
...
// SignerUI specifies what mgclod a UI needs to implement to be able to be used as a UI for the signer
type SignerUI interface {
	...
	// This mgclod can be used by a UI to keep track of e.g. how much has been sent to a particular recipient.
	OnApprovedTx(tx gclapi.SignTransactionResult)
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, mgclodSelector *string) (*gclapi.SignTransactionResult, error) {
	var (
		err    error
		result SignTxResponse
	)
	msgs, err := api.validator.ValidateTransaction(&args, mgclodSelector)
	if err != nil {
		return nil, err
	}
	...
	response := gclapi.SignTransactionResult{Raw: rlpdata, Tx: signedTx}
...
// OBS! This mgclod is removed from the public API. It should not be exposed on the external API
