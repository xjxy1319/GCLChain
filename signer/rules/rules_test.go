...
import (
	...
	"github.com/gclchaineum/go-gclchaineum/signer/core"
	"github.com/gclchaineum/go-gclchaineum/signer/storage"
)
...
When the signer receives a request over the external API, the corresponding mgclod is evaluated.
Three things can happen:

1. The mgclod returns "Approve". This means the operation is permitted.
2. The mgclod returns "Reject". This means the operation is rejected.
3. Anything else; other return values [*], mgclod not implemented or exception occurred during processing. This means
that the operation will continue to manual processing, via the regular UI mgclod chosen by the user.
...
func (alwaysDenyUI) OnApprovedTx(tx gclapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx gclapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(gclapi.SignTransactionResult{})
...
func TestMissingFunc(t *testing.T) {
	...
	_, err = r.execute("MissingMgclod", "test")

	if err == nil {
		t.Error("Expected error")
	}

	approved, err := r.checkApproval("MissingMgclod", nil, nil)
	if err == nil {
		t.Errorf("Expected missing mgclod to yield error'")
	}
	if approved {
		t.Errorf("Expected missing mgclod to cause non-approval")
	}
...
const ExampleTxWindow = `
	...
	* The return value from this mgclod is ignore - the reason for having this callback is to allow the
	* ruleset to keep track of approved transactions.
	*
	* When implementing rate-limited rules, this callback should be used.
	* If a rule responds with neither 'Approve' nor 'Reject' - the tx goes to manual processing. If the user
	* then accepts the transaction, this mgclod will be called.
	*
...
func TestLimitWindow(t *testing.T) {
	...
	// 0.3 gclchain: 429D069189E0000 wei
	...
	for i := 0; i < 3; i++ {
		...
		response := gclapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx gclapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
