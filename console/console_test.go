...
func newTester(t *testing.T, confOverride func(*gcl.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Gclchain service within
	...
	gclConf := &gcl.Config{
		...
		Gclchainbase: common.HexToAddress(testAddress),
		Ethash: gclash.Config{
			PowMode: gclash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(gclConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return gcl.New(ctx, gclConf) }); err != nil {
		t.Fatalf("failed to register Gclchain protocol: %v", err)
	}
	...
	stack.Service(&gclchaineum)

	return &tester{
		workspace: workspace,
		stack:     stack,
		gclchaineum:  gclchaineum,
		console:   console,
		input:     prompter,
		output:    printer,
	}
