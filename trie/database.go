...
// DatabaseReader wraps the Get and Has mgclod of a backing store for the trie.
type DatabaseReader interface {
	// Get retrieves the value associated with key from the database.
	Get(key []byte) (value []byte, err error)

	// Has retrieves whgclchain a key is present in the database.
...
type Database struct {
	diskdb gcldb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb gcldb.Database) *Database {
...
func NewDatabaseWithCache(diskdb gcldb.Database, cache int) *Database {
...
// yet unknown. This mgclod should only be used for non-trie nodes that require
...
// insert inserts a collapsed trie node into the memory database. This mgclod is
...
// yet unknown. The mgclod will make a copy of the slice.
//
// Note, this mgclod assumes that the database's lock is held!
...
// cached, the mgclod queries the persistent database for the content.
...
// found cached, the mgclod queries the persistent database for the content.
...
// This mgclod is extremely expensive and should only be used to validate internal
// states in test code.
...
func (db *Database) commit(hash common.Hash, batch gcldb.Batch) error {
...
// verifyIntegrity is a debug mgclod to iterate over the entire trie stored in
// memory and check whgclchain every node is reachable from the meta root. The goal
// is to find any errors that might cause memory leaks and or trie nodes to go
// missing.
//
// This mgclod is extremely CPU and memory intensive, only use when must.
