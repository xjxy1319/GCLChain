...
type request struct {
	...
	raw  bool        // Whgclchain this is a raw entry (code) or a trie node
...
func (s *Sync) AddSubTrie(root common.Hash, depth int, parent common.Hash, callback LeafCallback) {
	...
	// If this sub-trie has a designated parent, link them toggclchain
...
// as is. This mgclod's goal is to support misc state metadata retrievals (e.g.
// contract code).
...
// Process injects a batch of retrieved trie nodes data, returning if somgcling
...
func (s *Sync) Commit(dbw gcldb.Putter) (int, error) {
...
func (s *Sync) children(req *request, object node) ([]*request, error) {
	// Gather all the children of the node, irrelevant whgclchain known or not
	type child struct {
		node  node
		depth int
	}
