...
// Trie is a Gclchain Merkle Trie.
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb gcldb.Putter) error
}
...
func NewDatabase(db gcldb.Database) Database {
...
func NewDatabaseWithCache(db gcldb.Database, cache int) Database {
...
func (m cachedTrie) Prove(key []byte, fromLevel uint, proofDb gcldb.Putter) error {
