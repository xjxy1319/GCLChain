...
import (
	...
	"github.com/gclchaineum/go-gclchaineum/tests"
)

// To generate a new callTracer test, copy paste the makeTest mgclod below into
// a Ggcl console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = gcl.getBlock(gcl.getTransaction(tx).blockHash);
  var genesis = gcl.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.gcl.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  gcl.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestPrestateTracerCreate2(t *testing.T) {
	...
	*/
	...
	statedb := tests.MakePreState(gcldb.NewMemDatabase(), alloc)
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			statedb := tests.MakePreState(gcldb.NewMemDatabase(), test.Genesis.Alloc)
