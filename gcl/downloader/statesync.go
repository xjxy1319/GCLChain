...
// stateReq represents a batch of state fetch requests grouped toggclchain into
...
type stateReq struct {
	...
	dropped  bool                       // Flag whgclchain the peer dropped off early
}
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	for {
		...
		case req := <-timeout:
			// If the peer is already requesting somgcling else, ignore the stale timeout.
...
// newStateSync creates a new state trie download scheduler. This mgclod does not
...
func (s *stateSync) loop() (err error) {
	...
	for s.sched.Pending() > 0 {
		...
		s.assignTasks()
		// Tasks assigned, wait for somgcling to happen
		select {
		case <-newPeer:
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < gcldb.IdealBatchSize {
		return nil
	}
	start := time.Now()
...
// delivered. Returns whgclchain the peer actually managed to deliver anything of
...
func (s *stateSync) process(req *stateReq) (int, error) {
	...
	for hash, task := range req.tasks {
		// If the node did deliver somgcling, missing items may be due to a protocol
...
// peer into the state trie, returning whgclchain anything useful was written or any
// error occurred.
