...
// LightPeer encapsulates the mgclods required to synchronise with a remote light peer.
...
// Peer encapsulates the mgclods required to synchronise with a remote full peer.
type Peer interface {
	LightPeer
...
// lightPeerWrapper wraps a LightPeer struct, stubbing out the Peer-only mgclods.
...
func (p *peerConnection) FetchHeaders(from uint64, count int) error {
	...
	if p.version < 62 {
		panic(fmt.Sprintf("header fetch [gcl/62+] requested on gcl/%d", p.version))
	}
...
func (p *peerConnection) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [gcl/62+] requested on gcl/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [gcl/63+] requested on gcl/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchNodeData(hashes []common.Hash) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [gcl/63+] requested on gcl/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
// Lacks retrieves whgclchain the hash of a blockchain item is on the peers lacking
// list (i.e. whgclchain we know that the peer does not have it).
...
//
// The mgclod also sets the starting throughput values of the new peer to the
