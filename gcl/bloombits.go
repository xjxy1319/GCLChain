...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Gclchain
...
func (gcl *Gclchain) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-gcl.shutdownChan:
					return

				case request := <-gcl.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(gcl.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(gcl.chainDb, task.Bit, section, head); err == nil {
...
// for the Gclchain header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size    uint64               // section size to generate bloombits for
	db      gcldb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db gcldb.Database, size, confirms uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := gcldb.NewTable(db, string(rawdb.BloomBitsIndexPrefix))
