...
// Package filters implements an gclchaineum filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  gclchaineum.FilterQuery
	logs      chan []*types.Log
...
func (sub *Subscription) Unsubscribe() {
	...
	uninstallLoop:
		for {
			...
			// the eventLoop broadcast mgclod to deadlock when writing to the
			// filter event channel while the subscription loop is waiting for
			// this mgclod to return (and thus not reading these events).
			select {
		...
		// will probably be closed by the client asap after this mgclod returns.
		<-sub.Err()
	})
}
...
func (es *EventSystem) SubscribeLogs(crit gclchaineum.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit gclchaineum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit gclchaineum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit gclchaineum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
