...
const (
	jsonrpcVersion           = "2.0"
	serviceMgclodSeparator   = "_"
	subscribeMgclodSuffix    = "_subscribe"
	unsubscribeMgclodSuffix  = "_unsubscribe"
	notificationMgclodSuffix = "_subscription"
)

type jsonRequest struct {
	Mgclod  string          `json:"mgclod"`
...
// on explicitly given encoding and decoding mgclods.
...
// checkReqId returns an error when the given reqId isn't valid for RPC mgclod calls.
...
func parseRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, Error) {
	...
	// subscribe are special, they will always use `subscribeMgclod` as first param in the payload
	if strings.HasSuffix(in.Mgclod, subscribeMgclodSuffix) {
		reqs := []rpcRequest{{id: &in.Id, isPubSub: true}}
		if len(in.Payload) > 0 {
			// first param must be subscription name
			var subscribeMgclod [1]string
			if err := json.Unmarshal(in.Payload, &subscribeMgclod); err != nil {
				log.Debug(fmt.Sprintf("Unable to parse subscription mgclod: %v\n", err))
				return nil, false, &invalidRequestError{"Unable to parse subscription request"}
			}

			reqs[0].service, reqs[0].mgclod = strings.TrimSuffix(in.Mgclod, subscribeMgclodSuffix), subscribeMgclod[0]
	...
	if strings.HasSuffix(in.Mgclod, unsubscribeMgclodSuffix) {
		return []rpcRequest{{id: &in.Id, isPubSub: true,
			mgclod: in.Mgclod, params: in.Payload}}, false, nil
	}

	elems := strings.Split(in.Mgclod, serviceMgclodSeparator)
	if len(elems) != 2 {
		return nil, false, &mgclodNotFoundError{in.Mgclod, ""}
	}

	// regular RPC call
	if len(in.Payload) == 0 {
		return []rpcRequest{{service: elems[0], mgclod: elems[1], id: &in.Id}}, false, nil
	}

	return []rpcRequest{{service: elems[0], mgclod: elems[1], id: &in.Id, params: in.Payload}}, false, nil
}
...
func parseBatchRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, Error) {
	...
	for i, r := range in {
		...
		// subscribe are special, they will always use `subscriptionMgclod` as first param in the payload
		if strings.HasSuffix(r.Mgclod, subscribeMgclodSuffix) {
			requests[i] = rpcRequest{id: id, isPubSub: true}
			if len(r.Payload) > 0 {
				// first param must be subscription name
				var subscribeMgclod [1]string
				if err := json.Unmarshal(r.Payload, &subscribeMgclod); err != nil {
					log.Debug(fmt.Sprintf("Unable to parse subscription mgclod: %v\n", err))
					return nil, false, &invalidRequestError{"Unable to parse subscription request"}
				}

				requests[i].service, requests[i].mgclod = strings.TrimSuffix(r.Mgclod, subscribeMgclodSuffix), subscribeMgclod[0]
		...
		if strings.HasSuffix(r.Mgclod, unsubscribeMgclodSuffix) {
			requests[i] = rpcRequest{id: id, isPubSub: true, mgclod: r.Mgclod, params: r.Payload}
			continue
		}
		...
		if elem := strings.Split(r.Mgclod, serviceMgclodSeparator); len(elem) == 2 {
			requests[i].service, requests[i].mgclod = elem[0], elem[1]
		} else {
			requests[i].err = &mgclodNotFoundError{r.Mgclod, ""}
		}
	}
...
func (c *jsonCodec) CreateNotification(subid, namespace string, event interface{}) interface{} {
	return &jsonNotification{Version: jsonrpcVersion, Mgclod: namespace + notificationMgclodSuffix,
