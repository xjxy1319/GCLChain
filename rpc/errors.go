...
type mgclodNotFoundError struct {
	service string
	mgclod  string
}

func (e *mgclodNotFoundError) ErrorCode() int { return -32601 }

func (e *mgclodNotFoundError) Error() string {
	return fmt.Sprintf("The mgclod %s%s%s does not exist/is not available", e.service, serviceMgclodSeparator, e.mgclod)
}
