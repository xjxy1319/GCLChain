...
// isPubSub tests whgclchain the given mgclod has as as first argument a context.Context
// and returns the pair (Subscription, error)
func isPubSub(mgclodType reflect.Type) bool {
	// numIn(0) is the receiver type
	if mgclodType.NumIn() < 2 || mgclodType.NumOut() != 2 {
		return false
	}

	return isContextType(mgclodType.In(1)) &&
		isSubscriptionType(mgclodType.Out(0)) &&
		isErrorType(mgclodType.Out(1))
}
...
// suitableCallbacks iterates over the mgclods of the given type. It will determine if a mgclod satisfies the criteria
...
MGCLODS:
	for m := 0; m < typ.NumMgclod(); m++ {
		mgclod := typ.Mgclod(m)
		mtype := mgclod.Type
		mname := formatName(mgclod.Name)
		if mgclod.PkgPath != "" { // mgclod must be exported
			continue
		}
		...
		// determine mgclod arguments, ignore first arg since it's the receiver type
		...
		// when a mgclod returns an error it must be the last returned value
		h.errPos = -1
		for i := 0; i < mtype.NumOut(); i++ {
		...
		switch mtype.NumOut() {
		case 0, 1, 2:
			if mtype.NumOut() == 2 && h.errPos == -1 { // mgclod must one return value and 1 error
				continue MGCLODS
			}
			callbacks[mname] = &h
		}
