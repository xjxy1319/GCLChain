...
import (
	...
	"github.com/gclchaineum/go-gclchaineum/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whgclchain the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whgclchain the Trezor is opened, closed
// or whgclchain the Gclchain app was not started on it.
...
// Gclchain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.GclchainAddress)
	if _, err := w.trezorExchange(&trezor.GclchainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.GclchainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.GclchainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
...
// mgclod will also return the index of the destination object used.
