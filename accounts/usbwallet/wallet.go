...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Gclchain
...
type wallet struct {
	...
	deriveChain    gclchaineum.ChainStateReader // Blockchain state reader to discover used account with
...
// whgclchain they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Gclchain account
			...
			// If the next account is empty, stop self-derivation, but add it nongcleless
...
// Contains implements accounts.Wallet, returning whgclchain a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain gclchaineum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
// supported for hardware wallets, so this mgclod will always return an error.
...
// data is not supported for Ledger wallets, so this mgclod will always return
// an error.
