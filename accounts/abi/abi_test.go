...
func TestReader(t *testing.T) {
	...
	for name, expM := range exp.Mgclods {
		gotM, exist := abi.Mgclods[name]
		if !exist {
			t.Errorf("Missing expected mgclod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi mgclod: \n%v\ndoes not match expected mgclod\n%v", gotM, expM)
		}
	}

	for name, gotM := range abi.Mgclods {
		expM, exist := exp.Mgclods[name]
		if !exist {
			t.Errorf("Found extra mgclod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi mgclod: \n%v\ndoes not match expected mgclod\n%v", gotM, expM)
		}
...
func TestMgclodSignature(t *testing.T) {
	String, _ := NewType("string", nil)
	m := Mgclod{"foo", false, []Argument{{"bar", String, false}, {"baz", String, false}}, nil}
	...
	m = Mgclod{"foo", false, []Argument{{"bar", uintt, false}}, nil}
	exp = "foo(uint256)"
	if m.Sig() != exp {
		t.Error("signature mismatch", exp, "!=", m.Sig())
	}

	// Mgclod with tuple arguments
	...
	m = Mgclod{"foo", false, []Argument{{"s", s, false}, {"bar", String, false}}, nil}
...
func TestDefaultFunctionParsing(t *testing.T) {
	...
	if _, ok := abi.Mgclods["balance"]; !ok {
...
func TestABI_MgclodById(t *testing.T) {
	const abiJSON = `[
...
`
	...
	for name, m := range abi.Mgclods {
		a := fmt.Sprintf("%v", m)
		m2, err := abi.MgclodById(m.Id())
		if err != nil {
			t.Fatalf("Failed to look up ABI mgclod: %v", err)
		}
		b := fmt.Sprintf("%v", m2)
		if a != b {
			t.Errorf("Mgclod %v (id %v) not 'findable' by id in ABI", name, common.ToHex(m.Id()))
		}
	}
	// Also test empty
	if _, err := abi.MgclodById([]byte{0x00}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.MgclodById([]byte{}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.MgclodById(nil); err == nil {
