...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Ggcl.LightScryptN, Ggcl.LightScryptP);
		...
		try {
			...
			// data from this mgclod invocation is a JSON encoded, encrypted key-file.
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.gclchaineum", new NodeConfig());
			node.start();
			...
			GclchainClient ec = node.getGclchainClient();
	...
	// signatures too. Regression test for go-gclchaineum issue #14599.
...
//
// This mgclod has been adapted from golang.org/x/mobile/bind/java/seq_test.go/runTest
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altoggclchain
	...
	workspace, err := ioutil.TempDir("", "ggcl-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/gclchaineum/ggcltest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Ggcl and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.gclchaineum", "github.com/gclchaineum/go-gclchaineum/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "ggcl.aar"), "ggcl.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "gclchaineum", "ggcltest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.gclchaineum.ggcltest"
