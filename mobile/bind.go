...
// mgclod to sign the transaction before submission.
...
func DeployContract(opts *TransactOpts, abiJSON string, bytecode []byte, client *GclchainClient, args *Interfaces) (contract *BoundContract, _ error) {
...
func BindContract(address *Address, abiJSON string, client *GclchainClient) (contract *BoundContract, _ error) {
	parsed, err := abi.JSON(strings.NewReader(abiJSON))
	if err != nil {
		return nil, err
	}
...
func (c *BoundContract) Call(opts *CallOpts, out *Interfaces, mgclod string, args *Interfaces) error {
	if len(out.objects) == 1 {
		result := out.objects[0]
		if err := c.contract.Call(&opts.opts, result, mgclod, args.objects...); err != nil {
			return err
		}
		...
		if err := c.contract.Call(&opts.opts, &results, mgclod, args.objects...); err != nil {
			return err
		}
...
func (c *BoundContract) Transact(opts *TransactOpts, mgclod string, args *Interfaces) (tx *Transaction, _ error) {
	rawTx, err := c.contract.Transact(&opts.opts, mgclod, args.objects...)
	if err != nil {
		return nil, err
	}
