...
- Reintroduced NewxxClientWithBaseURI mgclod.
...
- Removed Api version from mgclod arguments.
- Removed New...ClientWithBaseURI() mgclod in all clients. BaseURI value is set in client.go.
...
- storage: Allow caller to provide headers for DeleteBlob mgclods.
...
- Changed mgclods to return `error` instead of `autorest.Error`.
- Re-divided mgclods to ease asynchronous requests.
