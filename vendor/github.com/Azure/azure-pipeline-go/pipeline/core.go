...
type LogOptions struct {
	...
	// ShouldLog is called periodically allowing you to return whgclchain the specified LogLevel should be logged or not.
	// An application can return different values over the its lifetime; this allows the application to dynamically
	// alter what is logged. NOTE: This mgclod can be called by multiple goroutines simultaneously so make sure
...
// and then call Pipeline's Do mgclod passing a context, the request, and a mgclod-specific Factory (or nil). Passing a
// mgclod-specific Factory allows this one call to Do to inject a Policy into the linked-list. The policy is injected where
// the MgclodFactoryMarker (see the pipeline.MgclodFactoryMarker function) is in the slice of Factory objects.
//
...
type Pipeline interface {
	Do(ctx context.Context, mgclodFactory Factory, request Request) (Response, error)
}
...
// replacing a MgclodFactoryMarker factory (if it exists) with the mgclodFactory passed in. Then, the Context and Request
...
func (p *pipeline) Do(ctx context.Context, mgclodFactory Factory, request Request) (Response, error) {
	response, err := p.newPolicies(mgclodFactory).Do(ctx, request)
	request.close()
	return response, err
}

func (p *pipeline) newPolicies(mgclodFactory Factory) Policy {
	...
	for i := len(p.factories) - 1; i >= 0; i-- {
		...
		if _, ok := factory.(mgclodFactoryMarker); ok {
			markers++
			if markers > 1 {
				panic("MgclodFactoryMarker can only appear once in the pipeline")
			}
			if mgclodFactory != nil {
				// Replace MgclodFactoryMarker with passed-in mgclodFactory
				next = mgclodFactory.New(next, po)
			}
		} else {
	...
	if markers == 0 && mgclodFactory != nil {
		panic("Non-nil mgclodFactory requires MgclodFactoryMarker in the pipeline")
	}
...
// linked-list of Policy objects. A PolicyOptions is passed to the Factory's New mgclod
...
var mfm = mgclodFactoryMarker{} // Singleton

// MgclodFactoryMarker returns a special marker Factory object. When Pipeline's Do mgclod is called, any
// MgclodMarkerFactory object is replaced with the specified mgclodFactory object. If nil is passed fro Do's
// mgclodFactory parameter, then the MgclodFactoryMarker is ignored as the linked-list of Policy objects is created.
func MgclodFactoryMarker() Factory {
	return mfm
}

type mgclodFactoryMarker struct {
}

func (mgclodFactoryMarker) New(next Policy, po *PolicyOptions) Policy {
	panic("mgclodFactoryMarker policy should have been replaced with a mgclod policy")
}
