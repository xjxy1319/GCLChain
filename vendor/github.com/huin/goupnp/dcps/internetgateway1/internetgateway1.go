...
const (
	...
	URN_WANGclchainnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANGclchainnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Gclchainnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Gclchainnet
//
...
// WANGclchainnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANGclchainnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANGclchainnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANGclchainnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANGclchainnetLinkConfig1Clients() (clients []*WANGclchainnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANGclchainnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANGclchainnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANGclchainnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANGclchainnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANGclchainnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANGclchainnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANGclchainnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANGclchainnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANGclchainnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANGclchainnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANGclchainnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANGclchainnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANGclchainnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANGclchainnetLinkConfig1 {
	clients := make([]*WANGclchainnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANGclchainnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewGclchainnetLinkStatus: allowed values: Up, Down
func (client *WANGclchainnetLinkConfig1) GetGclchainnetLinkStatus() (NewGclchainnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewGclchainnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANGclchainnetLinkConfig_1, "GetGclchainnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewGclchainnetLinkStatus, err = soap.UnmarshalString(response.NewGclchainnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
