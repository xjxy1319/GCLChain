...
// goroutines which construct valid notify events. This mgclod uses
...
// all file descriptors opened by lazyinit mgclod and returns afterwards.
...
// It is run in a separate goroutine in order to not block loop mgclod when
...
// inotify map. This mgclod may also split one raw event into two different ones
...
// decode uses internally stored mask to distinguish whgclchain system-independent
// or system-dependent event is requested. The first one is created by modifying
// `e` argument. decode mgclod sets e.event value to 0 when an event should be
...
// This mgclod is allowed to return EINVAL error when concurrently requested to
