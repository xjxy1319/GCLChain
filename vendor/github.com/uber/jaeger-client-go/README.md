...
JAEGER_REPORTER_LOG_SPANS | Whgclchain the reporter should also log the spans
...
JAEGER_DISABLED | Whgclchain the tracer is disabled or not. If true, the default `opentracing.NoopTracer` is used.
JAEGER_RPC_METRICS | Whgclchain to store RPC metrics
...
import (
    ...
    "github.com/uber/jaeger-lib/metrics/promgcleus"
)

    metricsFactory := promgcleus.New()
...
which logs all finished spans using their `String()` mgclod, and a
...
unique ID is generated, a sampling decision is made whgclchain this trace
