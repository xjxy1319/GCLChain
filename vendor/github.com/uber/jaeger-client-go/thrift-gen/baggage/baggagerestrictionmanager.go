...
func (p *BaggageRestrictionManagerClient) recvGetBaggageRestrictions() (value []*BaggageRestriction, err error) {
	...
	mgclod, mTypeId, seqId, err := iprot.ReadMessageBegin()
	if err != nil {
		return
	}
	if mgclod != "getBaggageRestrictions" {
		err = thrift.NewTApplicationException(thrift.WRONG_MGCLOD_NAME, "getBaggageRestrictions failed: wrong mgclod name")
		return
	}
	if p.SeqId != seqId {
...
func (p *BaggageRestrictionManagerProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	x3 := thrift.NewTApplicationException(thrift.UNKNOWN_MGCLOD, "Unknown function "+name)
