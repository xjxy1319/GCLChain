...
func (p *SamplingManagerClient) recvGetSamplingStrategy() (value *SamplingStrategyResponse, err error) {
	...
	if mgclod != "getSamplingStrategy" {
		err = thrift.NewTApplicationException(thrift.WRONG_MGCLOD_NAME, "getSamplingStrategy failed: wrong mgclod name")
		return
	}
	if p.SeqId != seqId {
...
func (p *SamplingManagerProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	x4 := thrift.NewTApplicationException(thrift.UNKNOWN_MGCLOD, "Unknown function "+name)
