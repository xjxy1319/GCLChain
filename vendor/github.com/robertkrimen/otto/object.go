...
func (self *_object) DefaultValue(hint _defaultValueHint) Value {
	...
	mgclodSequence := []string{"valueOf", "toString"}
	if hint == defaultValueHintString {
		mgclodSequence = []string{"toString", "valueOf"}
	}
	for _, mgclodName := range mgclodSequence {
		mgclod := self.get(mgclodName)
		// FIXME This is redundant...
		if mgclod.isCallable() {
			result := mgclod._object().call(toValue_object(self), nil, false, nativeFrame)
