...
// Call mgclod on IDispatch with parameters.
func (d *Dispatch) Call(mgclod string, params ...interface{}) (result *VARIANT, err error) {
	id, err := d.GetId(mgclod)
	if err != nil {
		return
	}

	result, err = d.Invoke(id, DISPATCH_MGCLOD, params)
	return
}

// MustCall mgclod on IDispatch with parameters.
func (d *Dispatch) MustCall(mgclod string, params ...interface{}) (result *VARIANT) {
	id, err := d.GetId(mgclod)
	if err != nil {
		panic(err)
	}
