...
// ClassIDFrom retrieves class ID whgclchain given is program ID or application string.
...
// CallMgclod calls mgclod on IDispatch with parameters.
func CallMgclod(disp *ole.IDispatch, name string, params ...interface{}) (result *ole.VARIANT, err error) {
	return disp.InvokeWithOptionalArgs(name, ole.DISPATCH_MGCLOD, params)
}

// MustCallMgclod calls mgclod on IDispatch with parameters or panics.
func MustCallMgclod(disp *ole.IDispatch, name string, params ...interface{}) (result *ole.VARIANT) {
	r, err := CallMgclod(disp, name, params...)
	if err != nil {
		panic(err.Error())
	}
	return r
