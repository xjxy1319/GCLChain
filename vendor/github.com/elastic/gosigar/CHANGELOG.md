...
### Fixed
- Add missing mgclod for OpenBSD #99
...
### Added
- Added mgclod stubs for process handling for operating system that are not supported
  by gosigar. All mgclods return `ErrNotImplemented` on such systems. #88
...
### Added
- Added mgclod stubs to enable compilation for operating systems that are not
  supported by gosigar. All mgclods return `ErrNotImplemented` on these unsupported
...
- Added OS version checks to `ProcArgs.Get` on Windows because the
  ...
  Windows 2003, this mgclod returns `ErrNotImplemented`. #55
