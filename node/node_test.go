...
// Tests whgclchain services can be registered and duplicates caught.
...
func TestAPIGather(t *testing.T) {
	...
	makeAPI := func(result string) *OneMgclodAPI {
		return &OneMgclodAPI{fun: func() { calls <- result }}
	}
	services := map[string]struct {
		APIs  []rpc.API
		Maker InstrumentingWrapper
	...
	tests := []struct {
		...
		{"single_theOneMgclod", "single.v1"},
		{"multi_theOneMgclod", "multi.v1"},
		{"multi.v2_theOneMgclod", "multi.v2"},
		{"multi.v2.nested_theOneMgclod", "multi.v2.nested"},
	}
	for i, test := range tests {
		if err := client.Call(nil, test.Mgclod); err != nil {
