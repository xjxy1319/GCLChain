...
import (
	...
	"github.com/promgcleus/promgcleus/util/flock"
)
...
func New(conf *Config) (*Node, error) {
	...
	// Ensure that the AccountManager mgclod works before the node has started.
	// We rely on this in cmd/ggcl.
...
// startRPC is a helper mgclod to start all the various RPC endpoint during node
...
// at the time of invocation, the mgclod immediately returns.
func (n *Node) Wait() {
	n.lock.RLock()
	if n.server == nil {
		n.lock.RUnlock()
...
// Server retrieves the currently running P2P network layer. This mgclod is meant
...
func (n *Node) OpenDatabase(name string, cache, handles int) (gcldb.Database, error) {
	if n.config.DataDir == "" {
		return gcldb.NewMemDatabase(), nil
	}
	return gcldb.NewLDBDatabase(n.config.ResolvePath(name), cache, handles)
}
