...
// as well as utility mgclods to operate on the service environment.
...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (gcldb.Database, error) {
	if ctx.config.DataDir == "" {
		return gcldb.NewMemDatabase(), nil
	}
	db, err := gcldb.NewLDBDatabase(ctx.config.ResolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
