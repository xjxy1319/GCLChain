...
func newFreeClientPool(db gcldb.Database, connectedLimit, totalLimit int, clock mclock.Clock) *freeClientPool {
	pool := &freeClientPool{
		db:             db,
		clock:          clock,
...
func (f *freeClientPool) connect(address string, disconnectFn func()) bool {
	...
	// check whgclchain (linUsage+connectedBias) is smaller than the highest entry in the connected pool
