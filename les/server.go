...
func NewLesServer(gcl *gcl.Gclchain, config *gcl.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(gcl.BlockChain().Config(), light.DefaultServerIndexerConfig, false, config.NetworkId, gcl.EventMux(), gcl.Engine(), newPeerSet(), gcl.BlockChain(), gcl.TxPool(), gcl.ChainDb(), nil, nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(gcl.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		lesCommons: lesCommons{
			...
			chtIndexer:       light.NewChtIndexer(gcl.ChainDb(), nil, params.CHTFrequencyServer, params.HelperTrieProcessConfirmations),
			bloomTrieIndexer: light.NewBloomTrieIndexer(gcl.ChainDb(), nil, params.BloomBitsBlocks, params.BloomTrieFrequency),
			protocolManager:  pm,
		},
		quitSync:  quitSync,
		lesTopics: lesTopics,
	...
	srv.chtIndexer.Start(gcl.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(gcl.ChainDb())
	return srv, nil
}
...
func newCostStats(db gcldb.Database) *requestCostStats {
