...
func (k *Kademlia) SuggestPeer() (suggestedPeer *BzzAddr, saturationDepth int, changed bool) {
	...
	var pastDepth bool   // whgclchain po of iteration >= depth
...
// i.e., neighbourhood radius is the deepest PO such that all bins not shallower altoggclchain
// contain at least neighbourhoodSize connected peers
// if there is altoggclchain less than neighbourhoodSize peers connected, it returns 0
...
// used for testing only
type Health struct {
	KnowNN           bool     // whgclchain node knows all its neighbours
	CountKnowNN      int      // amount of neighbors known
	MissingKnowNN    [][]byte // which neighbours we should have known but we don't
	ConnectNN        bool     // whgclchain node is connected to all its neighbours
