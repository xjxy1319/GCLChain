...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.gcl"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.gcl"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.gcl"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	gclAddr := "swarm.gcl"
	gclHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	gclResolve := newTestResolveValidator(gclHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(gclResolve, "")),
			addr:   gclAddr,
			result: gclHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(gclResolve, ""),
				MultiResolverOptionWithResolver(gclResolve, ""),
			),
			addr:   gclAddr,
			result: gclHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(gclResolve, "gcl"),
			),
			addr:   gclAddr,
			result: gclHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "gcl"),
				MultiResolverOptionWithResolver(doesntResolve, "gcl"),
				MultiResolverOptionWithResolver(gclResolve, "gcl"),
			),
			addr:   gclAddr,
		...
		},
		{
			...
			addr:   gclAddr,
			result: gclHash,
			err:    errors.New(`DNS name not found: "swarm.gcl"`),
		},
		{
