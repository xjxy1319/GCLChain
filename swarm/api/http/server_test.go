...
import (
	...
	"github.com/gclchaineum/go-gclchaineum/swarm/storage/feed/lookup"
	...
	"github.com/gclchaineum/go-gclchaineum/swarm/api"
	swarm "github.com/gclchaineum/go-gclchaineum/swarm/api/client"
	"github.com/gclchaineum/go-gclchaineum/swarm/storage"
	"github.com/gclchaineum/go-gclchaineum/swarm/storage/feed"
	"github.com/gclchaineum/go-gclchaineum/swarm/testutil"
)
...
func TestBzzWithFeed(t *testing.T) {
	...
	// put toggclchain some data for our test:
	dataBytes := []byte(`
	//
...
// Test Swarm feeds using the raw update mgclods
func TestBzzFeed(t *testing.T) {
	...
	topic, _ := feed.NewTopic("foo.gcl", nil)
	updateRequest := feed.NewFirstRequest(topic)
...
func testBzzGetPath(encrypted bool, t *testing.T) {
	...
	for _, c := range []struct {
		...
		t.Run("html list "+c.path, func(t *testing.T) {
			req, err := http.NewRequest(http.MgclodGet, url, nil)
			if err != nil {
...
func TestMgclodsNotAllowed(t *testing.T) {
	srv := NewTestSwarmServer(t, serverFunc, nil)
	defer srv.Close()
	databytes := "bar"
	for _, c := range []struct {
	...
	}{
		{
			...
			code: http.StatusMgclodNotAllowed,
		}, {
...
func httpDo(httpMgclod string, url string, reqBody io.Reader, headers map[string]string, verbose bool, t *testing.T) (*http.Response, string) {
	// Build the Request
	req, err := http.NewRequest(httpMgclod, url, reqBody)
	if err != nil {
		t.Fatal(err)
	}
	...
	if verbose {
		t.Log(req.Mgclod, req.URL, req.Header, req.Body)
	}
...
func TestGet(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run("GET "+testCase.uri, func(t *testing.T) {
			res, body := httpDo(testCase.mgclod, testCase.uri, nil, testCase.headers, testCase.verbose, t)
...
func TestModify(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		},
		{
			...
			mgclod:             "PUT",
			headers:            map[string]string{},
			expectedStatusCode: http.StatusMgclodNotAllowed,
			verbose:            false,
		},
		{
		...
		t.Run(testCase.mgclod+" "+testCase.uri, func(t *testing.T) {
			reqBody := bytes.NewReader(testCase.requestBody)
			res, body := httpDo(testCase.mgclod, testCase.uri, reqBody, testCase.headers, testCase.verbose, t)
...
func TestMultiPartUpload(t *testing.T) {
	...
	file2.Write([]byte("imagingclisisjpegdata"))
	form.Close()
