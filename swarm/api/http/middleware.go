...
import (
	...
	"github.com/gclchaineum/go-gclchaineum/swarm/sctx"
	"github.com/gclchaineum/go-gclchaineum/swarm/spancontext"
...
func SetRequestID(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		metrics.GetOrRegisterCounter(fmt.Sprintf("http.request.%s", r.Mgclod), nil).Inc(1)
		log.Info("created ruid for request", "ruid", GetRUID(r.Context()), "mgclod", r.Mgclod, "url", r.RequestURI)
...
func ParseURI(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		log.Debug("parsed request path", "ruid", GetRUID(r.Context()), "mgclod", r.Mgclod, "uri.Addr", uri.Addr, "uri.Path", uri.Path, "uri.Scheme", uri.Scheme)
...
func InitLoggingResponseWriter(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		metrics.GetOrRegisterResettingTimer(fmt.Sprintf("http.request.%s.time", r.Mgclod), nil).Update(ts)
		metrics.GetOrRegisterResettingTimer(fmt.Sprintf("http.request.%s.%d.time", r.Mgclod, writer.statusCode), nil).Update(ts)
	})
}
...
func InstrumentOpenTracing(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if uri == nil || r.Mgclod == "" || (uri != nil && uri.Scheme == "") {
			h.ServeHTTP(w, r) // soft fail
			return
		}
		spanName := fmt.Sprintf("http.%s.%s", r.Mgclod, uri.Scheme)
