...
The code tutorial [p2p programming in go-gclchaineum](https://github.com/nolash/go-gclchaineum-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `go-gclchaineum` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `go-gclchaineum` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
...
In `golang` as special mgclod is used:
...
returns:
1. whgclchain key was successfully removed (bool)
```
