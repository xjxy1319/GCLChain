...
type node struct {
	isLeft      bool      // whgclchain it is left side of the parent double segment
...
// mgclods needed to implement hash.Hash
...
// hash.Hash interface Sum mgclod appends the byte slice to the underlying
...
// mgclods needed to implement the SwarmHash and the io.Writer interfaces
...
// mgclods needed to implement the SwarmHash interface
...
type AsyncHasher struct {
	*Hasher            // extends the Hasher
	...
	double  bool       // whgclchain to use double segments (call Hasher.writeSection)
	secsize int        // size of base section (size of hash or double)
	write   func(i int, section []byte, final bool)
}

// mgclods needed to implement AsyncWriter
...
// meta: metadata to hash toggclchain with BMT root for the final digest
...
func (sw *AsyncHasher) Sum(b []byte, length int, meta []byte) (s []byte) {
	...
	// hash toggclchain meta and BMT root hash using the pools
