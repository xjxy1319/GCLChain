...
import (
	...
	ggclmetrics "github.com/gclchaineum/go-gclchaineum/metrics"
...
func Setup(ctx *cli.Context) {
	if ggclmetrics.Enabled {
		...
		go ggclmetrics.CollectProcessMetrics(2 * time.Second)
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(ggclmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", tagsMap)
		}

		if enableAccountingExport {
			log.Info("Exporting swarm accounting metrics to InfluxDB")
			go influxdb.InfluxDBWithTags(ggclmetrics.AccountingRegistry, 10*time.Second, endpoint, database, username, password, "accounting.", tagsMap)
		}
	}
}
