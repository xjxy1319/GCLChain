...
[https://swarm.gclchaineum.org](https://swarm.gclchaineum.org)

Swarm is a distributed storage platform and content distribution service, a native base layer service of the gclchaineum web3 stack. The primary objective of Swarm is to provide a decentralized and redundant store for dapp code and data as well as block chain and state data. Swarm is also set out to provide various base layer services for web3, including node-to-node messaging, media streaming, decentralised database services and scalable state-channel infrastructure for decentralised service economies.

[![Travis](https://travis-ci.org/gclchaineum/go-gclchaineum.svg?branch=master)](https://travis-ci.org/gclchaineum/go-gclchaineum)
[![Gitter](https://badges.gitter.im/Join%20Chat.svg)](https://gitter.im/gclchainsphere/orange-lounge?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge)
...
Building Swarm requires Go (version 1.10 or later).
    ...
    go get -d github.com/gclchaineum/go-gclchaineum

    go install github.com/gclchaineum/go-gclchaineum/cmd/swarm
...
To run Swarm you need an Gclchain account. You can create a new account by running the following command:
...
Once you have specified the password, the output will be the Gclchain address representing that account. For example:
...
### Gclchain Name Service resolution

The Gclchain Name Service is the Gclchain equivalent of DNS in the classic web. In order to use ENS to resolve names to Swarm content hashes (e.g. `bzz://theswarm.gcl`), `swarm` has to connect to a `ggcl` instance, which is synced with the Gclchain mainnet. This is done using the `--ens-api` flag.

    swarm --bzzaccount <your-account-here> \
          --ens-api '$HOME/.gclchaineum/ggcl.ipc'
...
You must have your working copy under `$GOPATH/src/github.com/gclchaineum/go-gclchaineum`.

Most likely you will be working from your fork of `go-gclchaineum`, let's say from `github.com/nirname/go-gclchaineum`. Clone or move your fork into the right place:

```
git clone git@github.com:nirname/go-gclchaineum.git $GOPATH/src/github.com/gclchaineum/go-gclchaineum
```
...
Testing only some mgclods:

```
go test -v -cpu 4 ./gcl -run TestMgclod
```

Note: here all tests with prefix TestMgclod will be run, so if you got TestMgclod, TestMgclod1, then both!
...
We have built a tool to help with automatic start of Grafana and InfluxDB and provisioning of dashboards at https://github.com/nonsense/statgcl , which requires that you have Docker installed.

Once you have `statgcl` installed, and you have Docker running locally, you have to:

1. Run `statgcl` and keep it running in the background
```
statgcl --rm --grafana-dashboards-folder $GOPATH/src/github.com/gclchaineum/go-gclchaineum/swarm/grafana_dashboards --influxdb-database metrics
```
...
Swarm offers a local HTTP proxy API that Dapps can use to interact with Swarm. The Gclchain Foundation is hosting a public gateway, which allows free access so that people can try Swarm without running their own node.
...
You can find a few reference Swarm decentralised applications at: https://swarm-gateways.net/bzz:/swarmapps.gcl

Their source code can be found at: https://github.com/gclchainsphere/swarm-dapps
...
complex changes though, please check up with the core devs first on [our Swarm gitter channel](https://gitter.im/gclchainsphere/orange-lounge)
 ...
 * [Code review guidelines](https://github.com/gclchaineum/go-gclchaineum/wiki/Code-Review-Guidelines).
