...
#
# The cluster contains a bootnode, a ggcl node and multiple swarm nodes, with
...
# static ggcl configuration
GGCL_IP="192.168.33.3"
GGCL_RPC_PORT="8545"
GGCL_RPC_URL="http://${GGCL_IP}:${GGCL_RPC_PORT}"
...
# network namespaces toggclchain
create_network() {
...
# start_bootnode starts a bootnode which is used to bootstrap the ggcl and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a ggcl node with --datadir pointing at <base-dir>/ggcl
# and a single, unlocked account with password "ggcl"
start_geth_node() {
  local dir="${base_dir}/ggcl"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get ggcl account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GGCL_IP}"
    --rpcport   "${GGCL_RPC_PORT}"
    --verbosity "6"
  )

  start_node "ggcl" "${GGCL_IP}" "$(which ggcl)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vgcl pair
  local vgcl0="vgcl${name}0"
  local vgcl1="vgcl${name}1"
  ip link add name "${vgcl0}" type vgcl peer name "${vgcl1}"

  # add one end to the bridge
  ip link set dev "${vgcl0}" master "${BRIDGE_NAME}"
  ip link set dev "${vgcl0}" up

  # add the other end to the namespace, rename it gcl0 and give it the ip
  ip link set dev "${vgcl1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vgcl1}" name "gcl0"
  ip netns exec "${name}" ip link set dev "gcl0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "gcl0"
}
...
create_account() {
  ...
  ggcl --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
