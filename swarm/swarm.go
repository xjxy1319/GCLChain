...
import (
	...
	"github.com/gclchaineum/go-gclchaineum/contracts/chequebook"
	"github.com/gclchaineum/go-gclchaineum/contracts/ens"
	...
	"github.com/gclchaineum/go-gclchaineum/p2p/protocols"
	...
	httpapi "github.com/gclchaineum/go-gclchaineum/swarm/api/http"
	"github.com/gclchaineum/go-gclchaineum/swarm/fuse"
	"github.com/gclchaineum/go-gclchaineum/swarm/log"
	"github.com/gclchaineum/go-gclchaineum/swarm/network"
	"github.com/gclchaineum/go-gclchaineum/swarm/network/stream"
	"github.com/gclchaineum/go-gclchaineum/swarm/pss"
	"github.com/gclchaineum/go-gclchaineum/swarm/state"
	"github.com/gclchaineum/go-gclchaineum/swarm/storage"
	"github.com/gclchaineum/go-gclchaineum/swarm/storage/feed"
	"github.com/gclchaineum/go-gclchaineum/swarm/storage/mock"
	"github.com/gclchaineum/go-gclchaineum/swarm/swap"
	"github.com/gclchaineum/go-gclchaineum/swarm/tracing"
)
...
func NewSwarm(config *api.Config, mockStore *mock.NodeStore) (self *Swarm, err error) {
	...
	if config.SwapAPI != "" && config.SwapEnabled {
		...
		backend, err = gclclient.Dial(config.SwapAPI)
		if err != nil {
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config, privkey *ecdsa.PrivateKey) (*ensClient, error) {
	...
	gclClient := gclclient.NewClient(client)
	...
	dns, err := ens.NewENS(transactOpts, ensRoot, gclClient)
	if err != nil {
		return nil, err
	}
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := gclclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
