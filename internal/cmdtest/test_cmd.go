...
type TestCmd struct {
	// For total convenience, all testing mgclods are available.
	*testing.T
...
// reexec init function for that name (e.g. "ggcl-test" in cmd/ggcl/run_test.go)
...
// This mgclod can also be called from an expect template, e.g.:
//
//     ggcl.expect(`Passphrase: {{.InputLine "password"}}`)
